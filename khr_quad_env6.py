...
class KHRQuadEnv:
    ...
    def _reward_torque_limits(self):
        # [v9-A] 定格トルクの soft 比を超えた分だけを罰する（飽和＝bang-bang制御を抑制）。
        # get_dofs_control_force は effort limit でクランプ済みなので、限界近傍に居ること自体を罰する。
        torques = self.robot.get_dofs_control_force(self.motors_dof_idx)
        soft = self.reward_cfg.get("torque_soft_ratio", 0.85) * self.torque_limit
        return torch.sum(torch.clamp(torch.abs(torques) - soft, min=0.0) ** 2, dim=1)
