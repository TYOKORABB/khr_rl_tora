"""四足歩行 学習スクリプト v2 — 「もっと速く・自然に」歩かせる改善版。

khr_train_quad.py (v1, exp=khr-quadruped) からのコピー。環境コード(khr_quad_env.py)は
変更せず、設定値のみ 3 箇所を変更した。目的は v1 の「遅い(実測 約0.036 vs 指令0.15 m/s)・
すり足」を、既存パラメータの調整だけで改善すること（新しい報酬関数は足さない）。

v1 → v2 の変更点:
  - tracking_sigma      0.25 → 0.15   速度追従を厳しく（指令速度に届かないと報酬が減る）
  - lin_vel_x_range  [0,0.2] → [0,0.4] より速い前進を指令できるように
  - feet_height_target 0.035 → 0.06   遊脚を高く上げさせ、すり足を減らして自然に

先行研究: Rudin et al. 2021 (legged_gym) の ablation で速度追従報酬が最重要と示される。
既定 exp_name は khr-quadruped2。eval/record/report スクリプトはそのまま流用可。
"""
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped2") # v2: 速さ+自然さ改善
