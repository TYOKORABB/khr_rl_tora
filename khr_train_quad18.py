...
khr_train_quad17.py (v17) のコピー。環境は khr_quad_env15.py。

v17 の結果:
  膝の周期運動は達成（ROM 2.7→**56.2°**、膝が足首を上回る主動力に復帰）。
  しかし代償として
   ① **トルクのピークが 63%→91%**(r_knee)。「100%は絶対NG」に対し余裕が9%しかない
   ② **追従が低下**（前進 75→62%、後退 69→49%、横 62/65→48/48%）
   ③ **接地の質が悪化**（後脚 足裏傾き 16.1→**21.4°**）＝つま先だけ／踵だけで接地している

v17 → v18 の変更点（2点。①②と③をそれぞれ狙う）:
  - `knee_swing_flexion` **3.0 → 1.8**。ROM 56° は過剰（v8 の 22° 程度が目安）。
    膝の運動量を適度に戻し、**トルクピークと追従を同時に改善**する狙い。
  - **足裏全面での接地**（ユーザ要望「踵だけ／つま先だけで歩かないようにしたい」）:
    - `feet_orientation` **-1.0 → -3.0**（罰を強化）
    - `feet_orientation_prelanding_phase = 0.85` を新設。接地中に加えて
      **着地直前（遊脚終盤）からも足裏水平を要求**する。接地時のみの罰では
      「傾いたまま振り下ろす」のを防げず、接地の瞬間に足裏が斜めになるため。
    - なお足裏傾きの指標はソール法線の水平成分であり、**ピッチ（つま先/踵）と
      ロール（内外の縁）の両方**を捉えるので、要望の測定にそのまま使える。

据え置き: 差分ベースの膝報酬（`_reward_knee_swing_flexion`, 目標+0.25rad, EMA α=0.05）、
脚別クリアランス目標（前0.012/後0.02）、`base_height` -3.0、トルク安全策
（torque_limits -5.0 / dof_pos_error -1.0）、leg_load_balance、heading_drift、feet_air_time。

注意（干渉リスク）: 着地直前に足裏水平を求めると膝の屈曲を抑制しうる。ただし
(a) 膝報酬は 1.8 で継続、(b) 対象は周期の終盤15%のみ、なので影響は限定的と見込む。
学習後に**膝 ROM が潰れていないか**を必ず確認する。

既定 exp_name は khr-quadruped18。v1〜v17 の成果物には一切触れない。
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped18") # v18: 足裏全面接地＋膝ROM適正化
