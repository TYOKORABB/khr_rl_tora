...
使い方:
  # Eureka 探索（小さめ設定で実験）
  python khr_train_eureka.py -e eureka-walking --iterations 3 --samples 4 \
      --rl-iters 100 --num-envs 4096 --eval-steps 500
  # 探索後に最良報酬で本学習まで通す
  python khr_train_eureka.py -e eureka-walking ... --final-train --final-iters 300

  # （内部用）1 候補の学習＋評価ワーカー
  python khr_train_eureka.py --worker --reward-file <path> --out <json> --rl-iters 100 ...
"""
import argparse
import datetime
import json
...
def main():
    ...
    # 候補あたりの RL 設定
    parser.add_argument("--rl-iters", type=int, default=100, help="候補1つの PPO 学習回数")
