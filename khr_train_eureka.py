...
目標は sim2real（実機 KHR-3HV/Meridian・50Hz で確実に動くこと）を最優先に、先輩の
ハンド設計歩行を越える「速い×滑らか×安定×頑健」な歩容を得ること。fitness は実機妥当な
±0.2 域の混合指令分布で survival*(0.40*progress + 0.15*yaw + 0.20*安定 + 0.25*滑らかさ)
で測る（滑らかさ=exp(-action_rate/6) を第一級に格上げ）。

使い方:
  # sim2real 志向の探索（既知の良報酬 walk-long を足場に warm-start して改良）
  python khr_train_eureka.py -e eureka-s2r --iterations 4 --samples 4 \
      --rl-iters 300 --num-envs 4096 --eval-steps 500 \
      --seed-reward logs/eureka-walk-long/eureka_reward.py
  # 探索後に最良報酬で本学習まで通す（実歩行は 800 iter 程度必要）
  python khr_train_eureka.py -e eureka-s2r ... --final-train --final-iters 800
...
def get_cfgs():
    ...
    command_cfg = {
        "num_commands": 3,
        # sim2real 重視: 実機(KHR-3HV/Meridian)で無理のない ±0.2 m/s に戻す（先輩ベースライン
        # と同じ土俵）。±0.3 の過速は動作のジッタ/転倒を誘発し実機転写に不利だったため。
        # 「速さ」はこの範囲での指令追従(progress)＋滑らかさで評価する。
...
def run_eureka(args):
    ...
    # 既知の良報酬から warm-start（sim2real 改良の足場）。指定報酬を先に評価し、その
    # fitness を初期ベスト・reflection の起点にすることで「その報酬を改良せよ」という
    # 探索になる（ゼロから再発見するより速く「先輩越え×滑らか」に到達しやすい）。
    if getattr(args, "seed_reward", None):
        seed_code = Path(args.seed_reward).read_text()
        print(f"[eureka] seed-reward で warm-start: {args.seed_reward}")
        seed_dir = base_dir / "seed"
        seed_dir.mkdir(exist_ok=True)
        seed_file = seed_dir / "reward.py"
        seed_file.write_text(seed_code)
        seed_res = _spawn_worker(seed_file, seed_dir / "result.json", args)
        if seed_res.get("ok"):
            best_overall = {"fitness": seed_res["fitness"], "code": seed_code, "iter": -1}
            (base_dir / "best_reward.py").write_text(seed_code)
            user_prompt = prompts.build_reflection_user_prompt(
                seed_code, build_feedback(seed_res, args.rl_iters))
            print(f"[eureka] seed fitness = {seed_res['fitness']:.4f}（これを上回る報酬を探索）")
        else:
            print(f"[eureka] seed 評価に失敗、通常の初期プロンプトで開始: "
                  f"{seed_res.get('error', '')[:200]}")
...
def main():
    ...
    # 既知の良報酬から探索を warm-start（先に評価してベスト/reflection の起点にする）
    parser.add_argument("--seed-reward", type=str, default=None,
                        help="この reward.py を足場に探索を開始(sim2real 改良向け)")
    # 内部: ワーカーモード
