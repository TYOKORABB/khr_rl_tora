"""四足歩行 学習スクリプト v22 — 左右対称性を「正しい量」で是正し、前進速度を回復する版。

khr_train_quad21.py (v21) のコピー。環境は khr_quad_env17.py。

v21 の結果:
  ✅ **トルク余裕を回復**（個体差ありでピーク 100% → **82%**、90%超の延べ時間 0.000%）
  ✅ **足裏の全面接地を回復**（13.3° → **7.8°**、v19 の 8.4° も上回る）
  ✅ 後退速度 0.260 m/s を維持
  ❌ **前進速度が 0.223 m/s に低下**（罰を3つ同時に強めた対価。目標 0.25 未達）
  ❌ **左右対称性が悪化**（後脚の接地率 左右差 9.5 → **14.0 pt**）＝ **対策が裏目**

v21 の失敗から得た診断（本版の設計根拠）:
  `leg_load_balance` を -1.0 → -2.0 と**強めたのに対称性は悪化した**。
  この報酬は **「左右後脚のトルク EMA の差」** を罰するものであり、
  **「接地時間(duty)の左右差」とは別の量**である。トルクを均衡させても接地タイミングは揃わない。
  → **「罰している量」と「直したい量」が一致していなかった。**
  これは v13（絶対高さ→接地基準の相対クリアランス）、v16（位置→変化）に続く
  **「報酬の“測る対象”を誤る」類型の3例目**である。

v21 → v22 の変更点（3点）:
  - [対称性・本命] `_reward_contact_duty_balance` を**新設**・`"contact_duty_balance": -10.0`。
    後脚の**接地duty（接地している時間の割合）の EMA の左右差**を直接罰する
    （α=0.01 ＝ 約4歩容周期の平均）。**直したい量そのものを測る。**
  - [対称性] `leg_load_balance` **-2.0 → -1.0 に戻す**（強めたら悪化したため）。
  - [速度] `tracking_lin_vel` **5.0 → 6.0**。トルク罰は据え置いたまま速度側の誘因を強め、
    v21 で失った前進速度（0.261→0.223）を取り戻す。

**据え置き（v21 で目標を達成したため触らない）**:
  `torque_soft_ratio` 0.60 / `torque_limits` -8.0（ピーク82%）、`feet_orientation` -4.5（足裏7.8°）。
**維持（v20/v19 の成果）**: `lin_vel_x_range` [-0.3,0.3]、`drift` の vx 成分、
  関節オフセットDR ±0.02rad、`tracking_sigma` 0.08。

監視項目:
  - **トルクのピーク（個体差あり）が 90% を超えないこと**（v21 は 82%。tracking を強めるため要注意）
  - 足裏傾き 7.8° を維持（10°以内）
  - 接地率の左右差が 14.0pt → **5pt 以内**へ（本版の主目的）
  - 前進 0.25 m/s 以上・後退 0.25 m/s 以上

既定 exp_name は khr-quadruped22。v1〜v21 の成果物には一切触れない。
...
def get_cfgs():
    ...
    reward_cfg = {
        ...
        "reward_scales": {
            "tracking_lin_vel": 6.0,  # v22: 5.0→6.0 v21で前進が0.223に落ちたため速度側の誘因を強める
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped22") # v22: 左右対称性の是正＋前進速度の回復
