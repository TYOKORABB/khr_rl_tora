...
from khr_quad_env6 import KHRQuadEnv  # v9: 上記 + トルク飽和/到達不能指令の罰(torque_limits/dof_pos_error)
...
def get_cfgs():
    ...
    reward_cfg = {
        ...
        "torque_soft_ratio": 0.85,  # v9-A: 定格トルクの85%を超えた分を _reward_torque_limits で罰する
        "reward_scales": {
            ...
            "dof_pos_error": -1.0,   # v9-B: PD目標角と実角の誤差を罰し「実現可能な指令」へ誘導（主対策）
            "torque_limits": -2.0,   # v9-A: 定格85%超のトルクを罰し飽和を抑制（補助）
    ...
    command_cfg = {
        ...
        "num_commands": 3,
        "lin_vel_x_range": [-0.2, 0.4],    # v9-E(③): 後退を開放（[0,0.4]→[-0.2,0.4]）。前進0.4/後退0.2
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped9") # v9: トルク飽和対策+後退
