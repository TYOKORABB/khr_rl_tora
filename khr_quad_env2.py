"""KHR-3HV 四足歩行環境（Genesis + rsl-rl / PPO）+ push 外乱版（v2 / 1b）。

khr_quad_env.py (v1〜v4 で使用) からのコピーに **push 外乱**を実装したもの。
Sim2Real の頑健性向上（Phase 1b）用。一定間隔ごとに胴体へランダム水平方向の
速度キックを与え、転倒回復を学ばせる（_push_robots 参照）。config の Mode_push_vel /
push_interval_s / max_push_vel_xy で制御し、Mode_push_vel=False なら無効＝v1 と同挙動。
これ以外の力学・観測・報酬は khr_quad_env.py と同一。v1〜v4 の env(khr_quad_env.py)には
一切触れないため過去の学習結果は完全に保持される。
...
class KHRQuadEnv:
    ...
    def _push_robots(self):
        ...
        大きさは [0, max_push_vel_xy] の一様乱数で、現在の base 並進速度(world x,y)に加算する。
        """
        ...
        mag = torch.rand(self.num_envs, device=self.device) * self.max_push_vel_xy
