...
khr_train_quad12.py (v12) のコピー。環境は khr_quad_env10.py。

v12 の実測で判明した足回りの問題（前進0.3, 8秒）:
  真のクリアランス(接地基準からの持ち上げ) 前脚 11〜14mm / 後脚 0〜5mm （目標60mm）
  接地中の滑り 前脚 0.126〜0.146 m/s（前進速度0.213 m/s の6〜7割＝引きずり）
  → 「足がほとんど上がらず地面を擦って進む」すり足歩容だった。

根本原因（設計上の欠陥）:
  _reward_feet_clearance が feet_height_target=0.06 を **世界座標の絶対高さ** として
  4本の足リンクに一律適用していた。しかし前脚(腕リンク)は立位で既に約0.10m、後脚は約0.03m と
  原点高さが異なる。前脚は「上げるほど目標から遠ざかり報酬が減る」＝**足上げを罰していた**。
  後脚も目標が遠く、トルク罰(v9〜)や滑らかさ罰に負けて上がらなかった。

v12 → v13 の変更点（環境 khr_quad_env10。姿勢・トロット・トルク安全策は v12 のまま）:
  - _reward_feet_clearance を **各足の接地時基準 foot_ref_z からの相対クリアランス** に修正。
    "foot_clearance_target": 0.03（4脚とも到達可能で意味のある目標）。scale 0.2→1.0。
  - _reward_feet_air_time を新設・"feet_air_time": 1.0。接地の瞬間に滞空時間(−0.2s)を加点し
    「上げて・運んで・置く」歩容を促す（legged_gym 定番）。更新は step 側で行い報酬関数は純粋。
  - "contact_no_vel" -0.2 → -1.0。接地中の引きずりを強く罰す。
  トルク安全策(torque_limits -5.0 等)は据え置き。v12のピークは定格の66%＝34%の余裕があり、
  足上げにトルクを払う余地は物理的にある（ただし飽和が出ないか学習後に必ず確認する）。

既定 exp_name は khr-quadruped13。v1〜v12 の成果物には一切触れない。
...
from khr_quad_env10 import KHRQuadEnv  # v13: 上記 + 相対クリアランス/滞空時間で足上げを促す
...
def get_cfgs():
    ...
    reward_cfg = {
        ...
        "foot_clearance_target": 0.03,  # v13: 各足の接地基準からの目標クリアランス[m]（4脚とも到達可能）
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped13") # v13: 足上げ改善(すり足解消)
