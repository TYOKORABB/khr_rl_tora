...
khr_train_quad16.py (v16) のコピー。環境は khr_quad_env14.py。

v16 の失敗（報酬ハック）:
  `_reward_knee_flexion` は膝の**絶対角度**（default からの屈曲量）を報酬にしていた。
  その結果、方策は **膝を常時 0.70rad（＝報酬の目標角度）で固定**して報酬だけ回収し、
  膝 ROM はむしろ 7.8° → **2.7°** に縮小した（swing−stance 差はわずか +0.3〜1.5°）。
  「膝を曲げる」ことは報酬にしたが「**膝を曲げ伸ばしする（動かす）**」ことは報酬にしていなかった。

v16 → v17 の変更点（変更は1点。v16 の報酬は削除して置き換え）:
  - `_reward_knee_swing_flexion` を新設・`"knee_swing_flexion": 3.0`。
    基準を **その脚自身の「接地時の膝角度」(EMA, α=0.05)** に取り、
    **遊脚中にそこから追加で曲がった分**だけを加点する（目標 +0.25rad、飽和線形）。
    → **定角度で固定すると差がゼロになり報酬も 0** になるため、v16 のハックは構造的に不可能。
    v13 で feet_clearance を絶対高さ→接地基準の相対量へ直したのと同じ発想。
  - v16 の `"knee_flexion"`（位置ベース）は **削除**（残すとハックが継続するため）。
  - scale は v16 の 2.0 より強い 3.0。v16 は一度動かせば済む静的な変化だったのに対し、
    v17 は毎周期 関節を動かす必要があり `dof_vel`/`action_rate` 等のコストを繰り返し払うため。

据え置き: 足裏水平の接地限定(v15)、脚別クリアランス目標（前0.012/後0.02）、`base_height` -3.0、
トルク安全策（torque_limits -5.0 / dof_pos_error -1.0）、leg_load_balance、heading_drift、feet_air_time。

注意: v16 で悪化した接地の質（足裏傾き 16.1°）は**本版では対処しない**。
着地直前に足裏水平を求めると膝の屈曲を再び抑制し主対策と干渉する恐れがあるため、
膝が動くことを確認してから次版で対処する。

既定 exp_name は khr-quadruped17。v1〜v16 の成果物には一切触れない。
...
from khr_quad_env14 import KHRQuadEnv  # v17: 上記 + 膝の swing-stance 差を報酬化（ハック耐性あり）
...
def get_cfgs():
    ...
    reward_cfg = {
        ...
        "reward_scales": {
            ...
            "knee_swing_flexion": 3.0,  # v17: 膝の swing-stance 差を報酬化（v16の位置ベースはハックされたため削除）
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped17") # v17: 膝のswing-stance差を報酬化
