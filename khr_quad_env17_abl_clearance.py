...
class KHRQuadEnv:
    ...
    def _reward_feet_clearance(self):
        """【ablation】v13 以前の「絶対高さ」版に差し戻したもの（比較用。本番では使わない）。

        目的: 「望む量を、その部位自身の基準からの相対量で測る」という設計則の効果を検証する。
        v13 でこの報酬を絶対高さ→接地基準からの相対量に直したが、それが本当に効いたのかは
        単一 seed の前後比較でしか示せていなかった。本 ablation は**形だけを元に戻し**、
        他の設定はすべて v23 baseline と同一にして、3 seed で統計的に判定する。

        原典: khr_quad_env.py（v1〜v12）の実装をそのまま復元。
          error = |target_feet_height(0.06) − 足の世界座標 z|、4脚一律。
        既知の問題（v13 の調査で判明）: 前脚(腕リンク)は立位で既に約 0.10m あり、
        上げるほど目標 0.06m から遠ざかって報酬が減る＝**足上げを罰する**形になっている。

        注意: v12 当時は scale も 0.2 だったが、本 ablation は**形の効果だけを見る**ため
        scale は baseline と同じ 1.0 のままにしてある（1 要因だけを変える）。
        """
        ...
        **足首に集中**し、「足首が膝より大トルク(30% > 19%)・膝ROMは9.6°」という
        ...
        for f, link_idx in enumerate(self.rear_feet_indices):
            ...
            if self.feet_orientation_stance_only:
                ...
                prelanding = (
                    self.leg_phase[:, self.rear_leg_phase_idx[f]] >= self.feet_orientation_prelanding_phase
                )
