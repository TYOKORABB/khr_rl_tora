"""【Ablation】v23 から `knee_swing_flexion` を除去した版（scale = 0.0）。

目的: 膝の周期運動（v17、swing-stance 差分報酬）の寄与を測る

- ベース設定は **v23 と完全に同一**（環境は khr_quad_env17.py）。変更は `knee_swing_flexion` の scale を 0.0 にする1点のみ。
...
既定 exp_name は khr-q23-abl-knee。v1〜v23 の成果物には一切触れない。
"""
...
def get_cfgs():
    ...
    reward_cfg = {
        ...
        "reward_scales": {
            ...
            "knee_swing_flexion": 0.0,  # [ABLATION:knee] 除去（元の値は khr_train_quad23.py 参照）
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-q23-abl-knee") # ablation: knee_swing_flexion を除去
