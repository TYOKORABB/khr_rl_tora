...
既定 exp_name は khr-quadruped27-p080。v1〜v23 の成果物には一切触れない。
...
from khr_quad_env19 import KHRQuadEnv  # v27: 環境は v25 と同一（設定のみ変更: prelanding 0.80）
...
def get_cfgs():
    ...
    obs_cfg = {
        ...
        "num_obs": 122,   # [v25] 累積ヘディング誤差を1次元追加
        "obs_scales": {
            "lin_vel": 2.0,
            "ang_vel": 0.25,
            "dof_pos": 1.0,
            "dof_vel": 0.05,
            "heading_err": 5.0,   # [v25] 定常0.157rad → 0.78 と他観測と同オーダーに揃える
        ...
        "obs_noise": {
            ...
            "dof_vel": 0.1, 
            "action" : 0.0,
            "heading_err": 0.25,   # [v25] ±0.25(scaled)=±0.05rad=±2.9°。実機のジャイロ積分誤差を想定
        }
    }
    reward_cfg = {
        ...
        "feet_orientation_prelanding_phase": 0.80,   # [v27] 0.85→0.80 着地に向けて足裏を水平に構える時間を延ばす  # v18: 着地直前(遊脚終盤)からも足裏水平を要求＝足裏全面で接地
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped27-p080") # v27: 着地直前の足裏水平要求を 0.85→0.80 に拡大
