"""四足歩行 学習スクリプト v21 — v20 の成果を保ちつつ、失ったトルク余裕と足裏接地を取り戻す版。

khr_train_quad20.py (v20) のコピー。**環境コードは変更せず設定値のみ**なので
環境は khr_quad_env16.py を流用する（v19 と同じ方針）。

v20 の結果（実機で指摘された3課題への対策）:
  ✅ ② 後退が遅い → **完全解決**（0.164 → 0.269 m/s、前進 0.266 とほぼ同等）
  ✅ ① 横へ逸れる → 改善（個体差なし 5%→3%、個体差±1.15°の8体で 12.3%±9.9 → 8.3%±6.5）
  🟡 ③ 旋回の中心ずれ → 左は大幅改善(0.033→0.005m)、右は悪化(0.024→0.062m)
  ❌ **トルクのピークが個体差ありで 100%**（v19 は同条件 82%）。「100%は絶対NG」に抵触
  ❌ **足裏の全面接地が後退**（v18 6.1° → v19 8.4° → v20 **13.3°**）
  ❌ 左右対称性の後退（後脚 接地率 左右差 3.0 → 9.5 pt）

v20 → v21 の変更点（**v20 の良い変更はすべて維持**し、失った3点を取り戻す数値調整）:
  - [トルク余裕の回復・最優先] `torque_soft_ratio` **0.65 → 0.60** ＋
    `torque_limits` **-5.0 → -8.0**。罰の対象範囲を広げ、かつ強度を上げる（合わせて約2倍）。
    目標は**個体差ありでもピーク90%未満**。
  - [足裏接地の回復] `feet_orientation` **-3.0 → -4.5**（1.5倍）。
    v18 で 6.1° を達成した設定（着地直前からの水平要求）はそのまま維持し、重みだけ強める。
  - [左右対称性の回復] `leg_load_balance` **-1.0 → -2.0**。
    ③の右旋回の悪化も左右非対称に起因する可能性があるため、ここにも効くことを期待する。

**維持する v20 の成果**: `lin_vel_x_range` [-0.3, 0.3]（②後退の対称化）、
`_reward_drift` の vx 成分（③旋回の中心ずれ）、関節オフセットDR ±0.02rad（①個体差への頑健性）。
**維持する v19 の成果**: `tracking_sigma` 0.08 / `tracking_lin_vel` 5.0（速度の限界価値）。

リスクと監視項目:
  - **罰を3つ同時に強めるため速度が落ちる恐れ**（v18 では feet_orientation を -3.0 に
    強めた結果、前進追従が 57% まで低下した前例がある）。ただし v19 の tracking 設定
    （sigma 0.08 / scale 5.0）が速度側の対抗力として残っているため、v18 当時とは条件が異なる。
  - 前進 0.266 m/s・後退 0.269 m/s を大きく落とさないこと。
  - 膝 ROM（v19 で 26〜30°）が潰れないこと。

既定 exp_name は khr-quadruped21。v1〜v20 の成果物には一切触れない。
...
from khr_quad_env16 import KHRQuadEnv  # v20: 上記 + 関節オフセットDR / drift に vx 追加
...
def get_cfgs():
    ...
    reward_cfg = {
        ...
        "reward_scales": {
            ...
            "leg_load_balance": -2.0,   # v21: -1.0→-2.0 左右対称性の回復（v20で接地率左右差 3.0→9.5pt に悪化）
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped21") # v21: v20の成果維持＋失った2点の回復
