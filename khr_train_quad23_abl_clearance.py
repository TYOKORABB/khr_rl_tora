...
khr_train_quad22.py (v22) のコピー。**環境コードは変更せず設定値のみ**なので
環境は khr_quad_env17.py を流用する（v19/v21 と同じ方針）。

v22 の結果:
  ✅✅ **後脚の接地率 左右差 14.0 → 2.2 pt**（全版で最良）。新設した
     `_reward_contact_duty_balance`（接地duty の EMA の左右差を直接罰す）が狙いどおり機能。
  ✅ 足裏傾き **6.4°**（v18 の 6.1° に迫る過去最良水準）、膝 ROM **28.3/26.1°**（理想レンジ）、
     後退 0.256 m/s、左旋回の中心ずれ 0.010 m。
  ❌ **トルクのピークが 82% → 93%（個体差あり）** と悪化し、要件「100%は絶対NG」の余裕を失った。
  ❌ 前進速度は 0.223 → **0.218** と**まったく改善しなかった**。

診断（v23 の設計根拠）:
  v22 の `tracking_lin_vel` **5.0 → 6.0** は、**速度を全く改善せずトルクのピークだけ +11pt
  押し上げた**。完全に割に合わない変更だった。
  理由は、速度を制限しているのが**追従報酬の弱さではなく、トルク罰（soft 0.60 / −8.0）と
  `feet_orientation` −4.5 という制約側**にあるため。誘因だけ強めても制約領域では速度は伸びず、
...
期待する結果:
  トルクのピークは v21 水準（**82%前後**）へ復帰。一方、対称性 2.2pt・足裏 6.4°・膝 27° は
  `tracking` とは独立な報酬によるものなので**維持される見込み**。
  → 実機投入の要件を満たしつつ、対称性と足裏で過去最良という構成を狙う。

既定 exp_name は khr-quadruped23-abl-clearance。v1〜v22 の成果物には一切触れない。
...
from khr_quad_env17_abl_clearance import KHRQuadEnv  # ablation: feet_clearance を v13以前の絶対高さ版に差し戻し
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped23-abl-clearance") # v23: v22の成果を保ちトルク余裕を回復
