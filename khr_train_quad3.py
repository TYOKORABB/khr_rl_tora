"""四足歩行 学習スクリプト v3 — 報酬バランスを是正して「実際に速く歩く」ようにする版。

khr_train_quad2.py (v2, exp=khr-quadruped2) からのコピー。環境コードは変更せず設定値のみ。

v2 の診断: 速度追従の判定を厳しくしても速くならなかった。報酬内訳を見ると、静止でも
前進追従報酬の約94%が得られる一方、動くと罰(acceleration/similar_to_default/action_rate)
が増えるため「歩く報酬 < 歩く罰」となり、静止が最適になっていた（探索は潰れていない）。

v3 の方針: 報酬バランスを是正し「歩くこと」を明確に割に合わせる（設定値4箇所のみ）。
sigma=0.15 / command[0,0.4] / feet_height=0.06 は v2 のまま維持。
  - tracking_lin_vel   1.5   → 4.0      指令速度に合わせる価値を大きく
  - similar_to_default -0.1  → -0.02    デフォルト姿勢維持の罰を弱める(歩行は姿勢を崩す)
  - action_rate        -0.05 → -0.02    能動的な足運びを許容
  - acceleration       -0.0001→-0.00002 動的な足運びを抑えすぎない(実効最大の罰を1/5に)

先行研究: Rudin et al. 2021 (legged_gym) の ablation で速度追従報酬が最重要と示される。
既定 exp_name は khr-quadruped3。eval/record/report スクリプトはそのまま流用可。
"""
...
def get_cfgs():
    ...
    command_cfg = {
        ...
        "num_commands": 3,
        "lin_vel_x_range": [0.0, 0.4],  # v2: 0.2→0.4 より速い前進を指令できるように
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped3") # v3: 報酬バランス是正
