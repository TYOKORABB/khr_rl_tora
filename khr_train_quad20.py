"""四足歩行 学習スクリプト v20 — 実機で観察された3つの課題に対策する版。

khr_train_quad19.py (v19) のコピー。環境は khr_quad_env16.py。

実機(2026-08-10, v19)で観察された課題と、調査で判明した原因:

 ① **前進指示なのに横へ逸れる**
    シミュ実測: 2.48m 進む間に横ずれ 0.126m（5%）。実機ではこれより悪化して見える。
    原因: シミュには無い**サーボのトリム誤差(0点ズレ)**。実機の KONDO サーボは個体ごとに
    0点が数度ずれており、同じ指令でも左右の実姿勢が食い違う。**DRに関節オフセットが無く**、
    学習時にこの誤差を一度も経験していなかった。

 ② **後退の速度が遅い**
    シミュ実測: 前進 0.248 m/s に対し後退 0.164 m/s（**34%遅い**）。
    原因: `lin_vel_x_range = [-0.2, 0.3]` と**指令範囲が前後非対称**だった
    （v9 で後退を -0.2 で開放し、v10 で前進のみ 0.4→0.3 に下げたため非対称が残った）。

 ③ **旋回する際に中心からずれる**
    シミュ実測: その場旋回10秒で中心が 0.033m ずれ、経路長は 0.30m（うろつく）。
    原因: `_reward_drift` が **vy と yaw しか見ておらず vx が入っていなかった**。
    旋回指令時は vx指令=0 だが、前後に流れても drift では罰されない状態だった。

v19 → v20 の変更点（3点。各々が上記①②③に1対1で対応）:
  - [③] `_reward_drift` に **vx 誤差を追加**（vx_err² + vy_err² + yaw_w·wz_err²）。
        旋回時の中心ずれ・横移動時の前後流れを直接罰する。
  - [②] `lin_vel_x_range` **[-0.2, 0.3] → [-0.3, 0.3]**（前後対称化）。
  - [①] **関節角オフセットのドメインランダム化を新設**
        `randomize_joint_offset: True`, `joint_offset_range: [-0.02, 0.02]`（±1.15度）。
        エピソードごとに各関節へ一定オフセットを与え PD 目標角に加算する（系統誤差として作用）。
        個体差があっても真っ直ぐ歩ける方策を狙う。**Sim2Real の観点でも本命の改善**。

据え置き: v19 の tracking 設定（sigma 0.08 / tracking_lin_vel 5.0）、足裏全面接地
（feet_orientation -3.0＋着地直前）、knee_swing_flexion 1.8、トルク安全策など。

監視項目:
  - トルクのピーク（v19 は 81%。「100%は絶対NG」のため 90% を超えないこと）
  - 足裏傾き 8.4°、膝 ROM 26〜30° が維持されること
  - 関節オフセットDRで学習が不安定化しないか（±0.02rad は控えめな設定）

既定 exp_name は khr-quadruped20。v1〜v19 の成果物には一切触れない。
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped20") # v20: 実機3課題の対策
