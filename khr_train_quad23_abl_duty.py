"""【Ablation】v23 から `contact_duty_balance` を除去した版（scale = 0.0）。

目的: 左右対称性（v22、接地duty の左右差）の寄与を測る

- ベース設定は **v23 と完全に同一**（環境は khr_quad_env17.py）。変更は `contact_duty_balance` の scale を 0.0 にする1点のみ。
...
既定 exp_name は khr-q23-abl-duty。v1〜v23 の成果物には一切触れない。
"""
...
def get_cfgs():
    ...
    reward_cfg = {
        ...
        "reward_scales": {
            ...
            "contact_duty_balance": 0.0,  # [ABLATION:duty] 除去（元の値は khr_train_quad23.py 参照）
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-q23-abl-duty") # ablation: contact_duty_balance を除去
