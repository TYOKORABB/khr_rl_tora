"""四足歩行 学習スクリプト v12 — yaw ドリフトを「持続偏りのEMA」で解消する版（v11失敗の修正）。

khr_train_quad11.py (v11) のコピー。環境は khr_quad_env9.py。

v11 の失敗（記録: experiments/khr-quadruped11/results.md）:
 - v11 は瞬時 yaw 誤差を横の5倍(yaw_drift_weight=5.0)で罰した。しかしトロットは前進時に
   本来 yaw が周期的に揺れるため、これを強く罰した結果「並進すると罰される→動かない」という
   degenerate 解に収束し、**前進28%/後退0%/横0%と並進が崩壊**（旋回のみ生存）。
 - 学び: 直進性を上げるには「瞬時 yaw」ではなく「持続的なヘディング偏り（時間平均）」を罰すべき。

v11 → v12 の変更点（環境 khr_quad_env9）:
  - _reward_heading_drift を新設・"heading_drift": -40.0。
    yaw 誤差(wz_err)の EMA（α=0.02, 約1s平均）＝じわじわ曲がる持続成分だけを罰す。
    周期的な揺れは平均で消えるため、並進を潰さずに直進性だけを上げられる見込み。
  - 瞬時 yaw 罰は v10 相当に戻す（yaw_drift_weight 5.0→1.0）。drift(-10) 等は v10 のまま。

方針: 本版で並進を保ったまま yaw を抑えられたら Sim2Real（符号キャリブ→実機投入→ギャップ実測）へ pivot。

既定 exp_name は khr-quadruped12。v1〜v11 の成果物には一切触れない。
...
from khr_quad_env9 import KHRQuadEnv  # v12: 上記 + 持続ヘディング偏り(wz誤差EMA)を罰す(heading_drift)
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped12") # v12: yawドリフト解消(EMA版)
