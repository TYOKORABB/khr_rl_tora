"""四足歩行 学習スクリプト v16 — 遊脚期の膝屈曲を直接報酬にする版（犬のような脚の短縮）。

khr_train_quad15.py (v15) のコピー。環境は khr_quad_env13.py。

v15 で特定した真因:
  PD の**指令角**と**実測角**の可動域を比較したところ、膝の**指令 ROM 自体が 6.8〜7.4°**しかなく、
  実測はそれを上回っていた（追従率 110〜115%）。つまり **トルク不足や PD の追従限界ではなく、
  方策が膝を動かす指令を出していない**＝完全に報酬設計が原因と確定した
  （`torque_limits` は容疑から外れた）。
  理由: 遊脚は股関節の回転だけ(ROM 15°)で振られており、**膝の屈曲を直接求める報酬が存在しない**。
  一方で関節運動には広くコストがかかる（dof_vel / action_rate / action_smoothness2 /
  acceleration / dof_pos_error）。「**曲げる利益が無く、動かすコストだけある**」ため、
  方策は膝を固めて股関節だけで振る戦略に収束していた。犬・馬が遊脚期に膝を畳んで
  脚を短縮するのと正反対である。

v15 → v16 の変更点（変更は1点のみ＝ablation を明確化）:
  - `_reward_knee_flexion` を新設・`"knee_flexion": 2.0`。
    **後脚の膝が遊脚期に default(0.40rad) より曲がっているほど加点**する
    （目標 +0.30rad, sharpness 10）。接地期は対象外にして支持脚の剛性は保つ。
    膝は片方向可動（[0.0, 2.793] rad）なので default より大きい側が屈曲。
  - 他（足裏水平の接地限定、脚別クリアランス目標 前0.012/後0.02、base_height -3.0、
    トルク安全策、leg_load_balance、heading_drift、feet_air_time 等）はすべて v15 のまま。

注意: scale 2.0 は初期値。膝が暴れて着地の質やトルクが悪化するなら弱め、
効果が出なければ強める。v15 で悪化した接地の質(足裏傾き13.5°)も併せて監視する。

既定 exp_name は khr-quadruped16。v1〜v15 の成果物には一切触れない。
...
from khr_quad_env13 import KHRQuadEnv  # v16: 上記 + 遊脚期の膝屈曲報酬（犬のような脚の短縮）
...
def get_cfgs():
    ...
    reward_cfg = {
        ...
        "knee_flexion_target": 0.30,      # v16: 遊脚期に膝を default(0.40rad) から +0.30rad 曲げるのを目標
        "knee_flexion_sharpness": 10.0,   # v16: 目標付近で立ち上がる指数報酬の鋭さ
        ...
        "reward_scales": {
            ...
            "knee_flexion": 2.0,      # v16: 遊脚期の膝屈曲を直接報酬に（膝を使う利益を作る／初期値・要調整）
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped16") # v16: 遊脚期の膝屈曲報酬
