"""四足歩行 学習スクリプト v11 — 前進時の yaw ドリフト（首振り）を解消する版。

khr_train_quad10.py (v10) のコピー。環境は khr_quad_env8.py。

v10 の残課題（実測診断）:
 - トルク100%飽和は完全解消・左右対称化・横ドリフトほぼ0 まで到達。ただし前進時に
   **yaw(首振り)が +0.053 rad/s（約3°/秒、10秒で27度、旋回半径4m）残存**。
   実機で「まっすぐ歩かせても緩く弧を描く」直進性の問題。Sim2Real 前の最後のシミュ磨き。

v10 → v11 の変更点（環境 khr_quad_env8、変更は1点のみ＝ablationを明確化）:
  - _reward_drift の yaw 成分を "yaw_drift_weight": 5.0 で重み付け（横成分の5倍）。
    前進時の首振り(uncommanded yaw)を重点的に罰す。指令旋回時は wz_err≈0 なので
    旋回性能は損なわない。drift の scale(-10) と他の報酬は v10 のまま。

方針: 本版で yaw を抑えたら Sim2Real（符号キャリブ→実機投入→ギャップ実測）へ pivot。

既定 exp_name は khr-quadruped11。v1〜v10 の成果物には一切触れない。
...
from khr_quad_env8 import KHRQuadEnv  # v11: 上記 + drift の yaw 成分を重み付け(yaw_drift_weight)して首振りを抑制
...
def get_cfgs():
    ...
    reward_cfg = {
        ...
        "yaw_drift_weight": 5.0,    # v11: drift の yaw(首振り)成分を横成分の5倍で罰し、前進時の首振りを重点抑制
        "reward_scales": {
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="khr-quadruped11") # v11: yawドリフト解消
